...
void read_parameters(const char *fname, params *pptr, connect *cptr)
{
	...
	while(!feof(fptr))
	{
		...
		while(tokptr!=NULL)
		{
			...
			else if(strcmp(tokptr, "start length")==0)
			{
				tokptr = strtok(NULL, "=");
				pptr->start_length = atoi(tokptr);
			}
...
void build_network(data *dptr, params *pptr, connect *cptr, cArray **bptr)
{
	...
	//Here's the stuff for continuously logging the weight vectors
	
	dptr->wVec = (float *)malloc(pptr->num_E1_cells*sizeof(float));
	
	dptr->wVec_time = (float **)malloc(pptr->num_E1_cells*sizeof(float *));
	dptr->wVec_time[0] = (float *)malloc((pptr->num_E1_cells*(pptr->timesteps/100))*sizeof(float));
	
	for(idx=1; idx<pptr->num_E1_cells; idx++)
	{
		dptr->wVec_time[idx] = dptr->wVec_time[0] + (idx * (pptr->timesteps/100));
	}
...
void set_RC_weights(params *pptr, data *dptr, connect *cptr)
{
	...
	//Initialising RC weights to zero.

	for(cell=0; cell<pptr->num_E1_cells; cell++)
	{
		for(connection=0; connection<cptr->num_RC1_connections; connection++)
		{
			//cptr->RC1_weights[cell][connection] = ((float)rand() / (float)RAND_MAX) * 0.1;						
		
			cptr->RC1_weights[cell][connection] = 0.0000001;
			
			//presynaptic = cptr->RC1_connections[cell][connection];
			
			//if(presynaptic == cell)
			//{
			//	cptr->RC1_weights[cell][connection] = 0.0;
			//}
